...
public class SegmentationToolDialog extends JDialog 
implements ChangeListener, MouseListener, ActionListener, MouseMotionListener {
	...
	private JButton btn_erode3, btn_dilate3, btn_erode7, btn_dilate7, btn_clear;
	...
	private JPanel makeGroup_DisplayProperties() {
		...
		window_center_slider = new JSlider(JSlider.HORIZONTAL, model.min_value, model.max_value, model.window_center);
		...
		window_width_slider = new JSlider(JSlider.HORIZONTAL, 0, (model.max_value-model.min_value)*2, model.window_width);
	...
	private JPanel makeGroup_Thresholding() {
		...
		min_threshold_slider = new JSlider(JSlider.HORIZONTAL, model.min_value, model.max_value, model.regionGrowing.min_threshold);
		min_threshold_slider .setMajorTickSpacing((model.max_value - model.min_value)  / 10);
		...
		max_threshold_slider = new JSlider(JSlider.HORIZONTAL, model.min_value, model.max_value, model.regionGrowing.max_threshold);
		max_threshold_slider .setMajorTickSpacing((model.max_value - model.min_value)  / 10);
		...
		chk_useThresholds = new JCheckBox("pixel value thresholds:", model.regionGrowing.useThresholdRange);
		...
		JPanel row = new JPanel();
		row.setLayout(new BoxLayout(row, BoxLayout.X_AXIS));
		row.add(new JLabel("Pixel values: min="+model.min_value+" max="+model.max_value));
		row.add(Box.createHorizontalGlue());
		group.add(row);
		...
		addSliderWithTextField(row, min_threshold_slider,  min_threshold_txt, this);
		group.add(row);
						
		row = new JPanel();
		row.setLayout(new BoxLayout(row, BoxLayout.X_AXIS));
		row.add(new JLabel("Max: "));
		addSliderWithTextField(row, max_threshold_slider,  max_threshold_txt, this);
		group.add(row);
	...
	private JPanel makeGroup_Labels() {
		...
		chk_excludeSegmentedRegions = new JCheckBox("exclude already labeled regions");
	...
	public void updateViewController() {
		...
		chk_excludeSegmentedRegions.setSelected(model.regionGrowing.stopAtOtherSegmentedRegions);
	...
	public void updateModel() {
		...
		model.regionGrowing.useThresholdRange = this.chk_useThresholds.isSelected();
		...
		model.regionGrowing.stopAtOtherSegmentedRegions = this.chk_excludeSegmentedRegions.isSelected();
	...
	public void mouseClicked(MouseEvent e) {
		...
		PixelInfo startPixel = PixelInfo.createFromXY(x, y, model.getImageWidth(), model.getImageHeight());
