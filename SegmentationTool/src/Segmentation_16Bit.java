...
public class Segmentation_16Bit implements PlugInFilter {
	...
	// for visualization: which color/alpha to use for selection and pixels within threshold 
	...
	public int setup(String arg, ImagePlus imp) {
		...
		// this filter only does 16 bit images, and does not change the original image
	...
	public void run(ImageProcessor ip) {
		...
		// analyze the input image, find min_value and max_value, set data window
		findMinMaxPixelValue();
		calculateAutoWindow();
		
		// initially empty selection
		selection = new boolean[N];
		for(int i=0; i<N; i++) selection[i]=false;

		// segmented regions are stored in a label field
		this.labels = new byte[N];
		
		// init region grower
		regionGrowing = new RegionGrowing(ip.getWidth(),ip.getHeight(), (short[]) ip.getPixels(), labels, null);
		...
		// show it before dialog opens (so dialog can take over mouse in that window)
		outputImage.show();
		outputImage.updateAndDraw();
	
		// create and show the dialg window, which acts as parameter controller/view 
	...
	public int getNumPixels() { return getImageHeight()*getImageWidth(); }
	...
	private void calculateOutputImage() {
		...
		if(inputImage == null || inputImage.getProcessor() == null) {
			if(!messageAlreadyShown) {
				new MessageDialog(null, "Segmentation Error", "The input image is no longer available.");
		...
		if(outputImage==null || outputImage.getProcessor()==null)
			outputImage = NewImage.createRGBImage("Output Image",inputImage.getWidth(), inputImage.getHeight(), 1, 0);
		...
		for(int idx=0; idx<max; idx++) {
			...
			// if threshold checkbox is activated, replace pixels within threshold values with semi-transparent red
			...
			if(label != 0) {
				v = labelColors[label-1]; // label starts at 0, arrays start at 0 
			}
	...
	public void clearSelection() {
		...
		for(int i=0; i<N; i++) { this.selection[i] = false;  }
	}
	...
	public void clearLabel(byte labelIndex) {
		final int N = getNumPixels();
		for(int i=0; i<N; i++) { if( (labels[i] & 0xff) == labelIndex) labels[i]=0;  }
	}
	...
	public void saveSelectionAsLabel(byte labelIndex) {
		...
		for(int i=0; i<N; i++) { if( selection[i] ) labels[i]=labelIndex;  }
	}
	...
	public void selectLabel(byte labelIndex) {
		final int N = getNumPixels();
		clearSelection();
		for(int i=0; i<N; i++) { if( labels[i]==labelIndex ) selection[i]=true;  }	
	}

	/*
	 *  interpolate between two RGB colors rgb1 and rgb2
	 *  interpol is a weight between 0.0 and 1.0
	 */
	static int interpolRGB(int rgb1, int rgb2, double interpol) {
		byte red   = (byte) (((rgb1&0xff0000)>>16) * (1.0-interpol) + ((rgb2&0xff0000)>>16) * interpol);
		byte green = (byte) (((rgb1&0x00ff00)>>8 ) * (1.0-interpol) + ((rgb2&0x00ff00)>>8 ) * interpol);
		byte blue  = (byte) (((rgb1&0x0000ff)    ) * (1.0-interpol) + ((rgb2&0x0000ff)    ) * interpol);
		return ((red<<16) & 0xff0000) + ((green<<8) & 0x00ff00) + (blue & 0x0000ff);
	}
	
	// getter for the output image's window
	ImageWindow getOutputWindow() { return outputImage.getWindow(); }
